...
# I took out all the code that enabled me to generate code for mazes, because
# it never worked that well and it made things messy.
...
def newPop(popSize):
    ...
    for individual in xrange(popSize):
...
class Picobot:
    ...
    def visualize(self):
        ...
        visType = raw_input("What visualization type would you like to see?"\
                            "('ASCII', '2D', or '3D') ").lower()
        ...
        # 2D uses Turtle to draw the Picobot, and is therefore very slow.
            ...
            for row in xrange(len(self.array)):
                for box in xrange(len(self.array[row])):
            ...
            # TODO try to remove shapes/vectors dependency here.
...
# Using "__main__ trick".
if __name__ == "__main__":
    popSize = input("Please supply a population size: ")
    gens = input("Please supply a number of generations: ")
    GA(popSize, gens)
