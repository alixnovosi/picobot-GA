...
#These are used to make Picobot and Program objects,
ROWS = 23
COLUMNS = 23
STATES = 5
allowedPatterns = ['xxxx','Nxxx','NExx','NxWx','xxxS','xExS','xxWS','xExx',
                   'xxWx']
#and these are used for testing
TRIALS = 20
STEPS = 1000
MUTATIONRATE = 0.02
TOPFRACTION = 0.19
...
#it. It works well with an empty room, but I'm still working on the other types.

#Types: "Empty Room", "Maze"
GRID = "Empty Room"
...
def GA(popSize, numGen):
    ...
    for gen in range(numGen):
        ...
        #as values so they can be called back later if they happen to be the
        #best program.
        ...
        fitnessList.sort()
        fitnessList.reverse()
        #The best fitness is the first item in the list, and the total fitness
        #is the sum of the list.  It is averaged by dividing by the size of the
        #population. The best bot is also defined
...
def nextGen(survivorProp, fitnessDict, fitnessList, popSize):
    ...
    while len(newGen)<popSize:
        ...
        #It makes sure two random parents aren't the same parent.
        if parent1 != parent2:
...
def newPop(popSize):
    ...
    programList = []
    #It creates a random population of programs, up to a given population size.
    for individual in range(popSize):
        #The programs are created, randomized, added to a list, and returned at
        #the end.
        newProg = Program()
        newProg.randomize()
...
#The below is code that lets a maze solver be generated. It's still here
#because I might insert it back in. The GA doesn't produce good results for
#mazes, but it's fun to watch.
##        while square == "W":
##            randRow = random.choice(range(1, ROWS+1))
##            randCol = random.choice(range(1, COLUMNS+1))
##            square = testBot.array[randRow][randCol]
##        startingPositions.append((randRow, randCol))
##    for pos in startingPositions:
##        testBot = Picobot(pos[0], pos[1], program)
##        testBot.array[0][0] = "W"
...
def gridMaker(roomType):
    ...
    array = []
    #The function generates the room for an empty room. For a maze, the array
    ...
    if roomType == "Empty Room":
        ...
        for c in range(COLUMNS+2):
            row.append("W")
        array.append(row)
    #For a maze, the function just uses the stored maze array.
    elif roomType == "Maze":
        array = mazeArray
...
def keyMaker(roomType):
    ...
    if roomType == "Maze":
        roomPatterns = ["xEWx", "NxxS", "NExx", "NxWx", "xxWS", "xExS",
                        "xEWS", "NEWx", "NxWS", "NExS", "Nxxx", "xExx",
                        "xxWx", "xxxS", "xxxx"]
    else:
...
class Picobot:
    ...
    def step(self):
        ...
        surroundings = []
        #For each square surrounding the Picobot, the Picobot decides if it's a
        #wall or not. If it is, it appends the direction to its surroundings.
        #If it isn't, it appends an 'x'.
        for square in relevantSquares:
            if square[0]== "W":
                surroundings.append(square[1])
            else:
                surroundings.append("x")
        #The surroundings are turned into a string, the way the other functions
        #(and the online simulator) expects them to be.
        surroundings = "".join(surroundings)
        #The Picobot uses its program to decide what state it should switch to
        #next, and which direction it should move in. getMove returns state and
        #direction in the form ('N', 0).
    ...
    def visualize(self):
        ...
        #2D uses Turtle to draw the Picobot, and is therefore very slow.
            ...
            #For each step, 
            for step in range(steps):
                #the current Picobot position is overridden with a grey square,
                lastSquare = Vector(xPos, yPos)
                lastVisited = Square(width = 10, center = lastSquare, \
                                     color = "Gray")
                lastVisited.render()
        ...
        #The 3D rendering uses VPython.
            ...
            #Just like before, the walls are rendered as blue boxes, after being
            #scaled to account for array/VPython differences.  Because we are in
            #3D now, the board is set up so that it faces the user immediately,
            #for convenience. 
            for vector in wallVectors:
                vector.y-=ROWS/2.0 
                vector.x+=COLUMNS/2.0
            ...
            #It is made visible after rendering. It gets made invisible while we
            #render squares that have been moved over, so it appears to move.
...
#This is an array that defines a maze room in the way I define an empty room.
#I found it (formatted slightly differently, but easy to convert) on a page
#from Northwestern University. They also do Picobot projects, and this
#specifically came from an assignment to make Picobot from scratch. I'll
#try to find the website so I can give proper credit.  This is here because
#I've been playing around with generating code for maze rooms, which doesn't
#work nearly as well.
mazeArray = [['W', 'W', 'W', 'W', 'W', 'W', 'W', 'W', 'W', 'W', 'W', 'W', 'W',
...
drawTest=Picobot(10,10,goodProgram)
